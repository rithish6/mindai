...
const OptionWheel = ({
  ...
  const onChangeRef = useRef<((index: number, label: string) => void) | undefined>(onChange);
  ...
  useEffect(() => {
    // Sync external selected index changes (like route transitions)
  ...
  return (
    <div
      ...
      className={`${styles['option-wheel']} ${side === 'right' ? styles['option-wheel--right'] : ''} ${isDragging ? styles['option-wheel--dragging'] : ''} ${className}`}
      style={{
    ...
    >
      {items.map((label, index) => (
        <div
          ...
          className={`${styles['option-wheel__item']} ${selectedIndex === index ? styles['option-wheel__item--selected'] : ''}`}
